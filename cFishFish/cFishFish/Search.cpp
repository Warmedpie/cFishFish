...
int Search::PVS(int alpha, int beta, int depth, int ply_deep, Move prev) {
    ...
    Move threat_killer = TT.transposition_search((long)(threat_move.to().index() * 64 + threat_move.from().index())).best;

    //Play other moves
    if (alpha < beta) {
        std::vector<ScoredMove> ordered_captures = orderCaptures(table_move, threat_killer);
...
std::vector<ScoredMove> Search::orderCaptures(Move table, Move threat) {
