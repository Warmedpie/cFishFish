...
float Eval::whitePhase(Board* board) {
	...
	return std::max(0.0f, ((whiteMaterial - 10) / 21));
}
...
float Eval::blackPhase(Board* board) {
	...
	return std::max(0.0f, ((blackMaterial - 10) / 21));
}
...
bool Eval::attackedBy(Board* b, Square sq, PieceType pt, Color side) {
	...
	if (pt == PieceType::PAWN) {
		if (attacks::pawn(~side, sq) & b->pieces(PieceType::PAWN, side)) return true;
...
int Eval::PsqM(Board* board, Move m) {
	...
	if (p == Piece::WHITEPAWN) {
		float black_mg = blackPhase(board);
		return black_mg * (mgPawnTableWhite[m.to().index()] - mgPawnTableWhite[m.from().index()]) - (black_mg - 1) * (egPawnTableWhite[m.to().index()] - egPawnTableWhite[m.from().index()]);
	}
	if (p == Piece::WHITEKNIGHT) {
	...
	if (p == Piece::WHITEKING) {
		float black_mg = blackPhase(board);
		return black_mg * (mgKingTableWhite[m.to().index()] - mgPawnTableWhite[m.from().index()]) - (black_mg - 1) * (egKingTableWhite[m.to().index()] - egKingTableWhite[m.from().index()]);
	}
